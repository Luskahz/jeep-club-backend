...
class ToolsOpenApiIntegrationTest {
    ...
    void openApiDescribesCurrentMemberAndAdministrativeToolContracts() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$['paths']['/admin/tools/users/{userId}']['post']['description']")
                        .value(org.hamcrest.Matchers.containsString("DISABLED")))
                .andExpect(jsonPath("$['paths']['/admin/tools/users/{userId}']['post']['responses']['404']").exists())
                .andExpect(jsonPath("$['paths']['/admin/tools/users/{userId}']['post']['responses']['409']").exists())
